...
class NanonisController(nanonis_programming_interface):
    ...
    def BiasSpectrStart(self, save_base_name=""):
        """
        Starts a bias spectroscopy in the Bias Spectroscopy module and returns the measurement data.
        ...
        """
        # Prepare arguments
        get_data = 1  # 1=True, return data
        save_base_name_bytes = save_base_name.encode("utf-8")
        save_base_name_size = len(save_base_name_bytes)
        ...
        cursor = 0
        # Parse channel names
        channel_names_size = from_binary("int", response["body"][cursor : cursor + 4])
        cursor += 4
        num_channels = from_binary("int", response["body"][cursor : cursor + 4])
        cursor += 4

        channel_names = []
        for _ in range(num_channels):
            name_len = from_binary("int", response["body"][cursor : cursor + 4])
            cursor += 4
            name = from_binary("string", response["body"][cursor : cursor + name_len])
        ...
        cursor += 4
        data_cols = from_binary("int", response["body"][cursor : cursor + 4])
        cursor += 4

        data = np.empty((data_rows, data_cols), dtype=np.float32)
        ...
        # Parse parameters
        num_parameters = from_binary("int", response["body"][cursor : cursor + 4])
        cursor += 4
        parameters = []
        for _ in range(num_parameters):
            param = from_binary("float32", response["body"][cursor : cursor + 4])
