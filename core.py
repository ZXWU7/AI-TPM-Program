...
class NanonisController(nanonis_programming_interface):
    ...
    def ZLimitsGet(self):
        ...
        return {"high": parsedResponse["0"], "low": parsedResponse["1"]}
    ...
    def SignalsNamesGet(self):
        ...
        for i in range(name_number):
            ...
            cursor += 4
            name = from_binary("string", response["body"][cursor : cursor + name_size])
    ...
    def SignalInSlotGet(self):  # need to be rewritten
        ...
        for i in range(Number_of_Signals):
            single_names_size = from_binary(
                "int", response["body"][cursor : cursor + 4]
            )
            cursor += 4
            slot = from_binary(
                "string", response["body"][cursor : cursor + single_names_size]
            )
        ...
        for i in range(Signals_indexes_size):
            ...
            Signals_indexes[
                (from_binary("int", response["body"][cursor : cursor + 4]))
            ] = Signals_names[i]
            cursor += 4
    ...
    def ScanFrameSet(self, center_x, center_y, width, height, angle=0):
        ...
        print(
            "Scan.FrameSet: center_x: {}, center_y: {}, width: {}, height: {}, angle: {}".format(
    ...
    def ScanSpeedSet(
        self,
        Forward_linear_speed,
        Backward_linear_speed,
    ...
    def ScanPropsGet(self):
        ...
        cursor += 4
        Series_name = from_binary(
            "string", response["body"][cursor : cursor + Series_name_size]
        )
        cursor += Series_name_size
        Comment_size = from_binary("int", response["body"][cursor : cursor + 4])
        cursor += 4
        Comment = from_binary(
            "string", response["body"][cursor : cursor + Comment_size]
        )
        return {
    ...
    def ScanFrameData(self, channel_index, data_dir=1):
        ...
        """
        response = self.send(
            "Scan.FrameDataGrab", "uint32", channel_index, "uint32", data_dir
        )
        cursor = 0
        ...
        for i in range(Scan_data_row):
            for j in range(Scan_data_col):
                ...
                cursor += 4
        scan_direction = from_binary("uint32", response["body"][cursor : cursor + 4])
        cursor += 4
        print(
            "Scan.FrameDataGrab: channel_name: {}, Scan_data_row: {}, Scan_data_col: {}, scan_direction: {}".format(
                channel_name, Scan_data_row, Scan_data_col, scan_direction
            )
        )  ###############################################
        print(
            "Scan.FrameDataGrab: data shape: {}".format(data.shape)
        )  #############################################
        return {
            "data": data,
            "row": Scan_data_row,
    ...
    def FolMeSpeedGet(self):
        ...
        return {"Speed": parsedResponse["0"], "Custom speed": parsedResponse["1"]}
    ...
    def SignalValsGet(self, signal_index, wait=0):
        Signal_value = self.parse_response(
            self.send("Signals.ValGet", "int", signal_index, "uint32", wait), "float32"
        )
        return Signal_value
    ...
    def TipShaper(
        self,
        Switch_Off_Delay=0.05,
        Lift_Time_1=0.1,
        Bias_Setting_Time=0.06,
        Lift_Time_2=0.06,
        TipLift="-4n",
        Lifting_Bias=-1.0,
        Lifting_Height="2n",
        ...
        for i in range(stepstotarget):
            ...
            self.TipZSet(
                Z_current + (i + 1) * (TipLift / stepstotarget)
            )  # set the new Z position

            time.sleep(
                Lift_Time_1 / stepstotarget
            )  # wait for Lift_Time_1/stepstotarget time
        ...
        for i in range(stepstotarget):
            ...
            self.TipZSet(
                Z_current
                + TipLift
                + (i + 1) * (Lifting_Height - TipLift) / stepstotarget
            )  # set the new Z position → Lifting_Height

            time.sleep(
                Lift_Time_2 / stepstotarget
            )  # wait for Lift_Time_2/stepstotarget time
    ...
    def lineScanmode(self, lineScan_buffer, angle=0, abslute_1D=True):
        ...
        # print(ScanBuffer)
        if (
            ScanBuffer["Pixels"] != lineScan_buffer
            and ScanFrame["height"]
            != ScanFrame["width"] / ScanBuffer["Pixels"] * lineScan_buffer
        ):
            # only set 3 lines to scan, there will be some 'nan' vlues in the scan data if the scan lines is 1 or 2
            self.ScanBufferSet(
                ScanBuffer["Pixels"], lineScan_buffer, ScanBuffer["Channel_indexes"]
            )
        ...
        print("lineScan has been set")
        return {
            "Pixels": ScanBuffer["Pixels"],
            "width": ScanFrame["width"],
            "angle": angle,
            "Channel_indexes": ScanBuffer["Channel_indexes"],
        }

    def lineScanGet(self, signal_index, fit_line=True):
        while True:
            Scaninfo_for = self.ScanFrameData(
                signal_index, data_dir=1
            )  # 30 is the number of signal channel which present Z(m) in simulation mode
            Scaninfo_back = self.ScanFrameData(signal_index, data_dir=0)

            if (
                Scaninfo_for["data"][0][0] == Scaninfo_for["data"][0][0]
                or Scaninfo_for["data"][-1][-1] == Scaninfo_for["data"][-1][-1]
            ):
                if (
                    Scaninfo_for["data"][0][0] != Scaninfo_for["data"][0][0]
                ):  # the left top data is nan, which means the scan directer is 'up'!!!
                    for i in range(0, Scaninfo_for["row"], 1):
                        if (
                            Scaninfo_for["data"][i][0] == Scaninfo_for["data"][i][0]
                            and Scaninfo_for["data"][i][-1]
                            == Scaninfo_for["data"][i][-1]
                            and Scaninfo_back["data"][i][0]
                            == Scaninfo_back["data"][i][0]
                            and Scaninfo_back["data"][i][-1]
                            == Scaninfo_back["data"][i][-1]
                        ):
                            line_Scan_data_for = Scaninfo_for["data"][i]
                            line_Scan_data_back = Scaninfo_back["data"][i]
                            return {
                                "line_Scan_data_for": line_Scan_data_for,
                                "line_Scan_data_back": line_Scan_data_back,
                            }
                elif (
                    Scaninfo_for["data"][-1][-1] != Scaninfo_for["data"][-1][-1]
                ):  # the right bottom data is nan, which means the scan directer is 'down'!!!
    ...
    def lineScanMoniterGet(
        self, lineScan_buffer=3, both_direction=True
    ):  # lineScan_buffer should not be smaller than 3 and bigger than 50
        """
        Can be proved.
        """
        ...
        while True:
            Scaninfo_for = self.ScanFrameData(
                Z_m_indexes, data_dir=1
            )  # 30 is the number of signal channel which present Z(m), while 14 inm, real Scan
            if both_direction:
            ...
            if (
                ...
                for i in range(0, Scaninfo_for["row"], 1):
                    if (
                        Scaninfo_for["data"][i][0] == Scaninfo_for["data"][i][0]
                        and Scaninfo_for["data"][i][-1] == Scaninfo_for["data"][i][-1]
                        and Scaninfo_back["data"][i][0] == Scaninfo_back["data"][i][0]
                        and Scaninfo_back["data"][i][-1] == Scaninfo_back["data"][i][-1]
                    ):
                        line_Scan_data_for = Scaninfo_for["data"][i]
                        line_Scan_data_back = Scaninfo_back["data"][i]
    ...
    def SessionPathGet(self):
        ...
        cursor += 4
        Session_path = from_binary(
            "string", response["body"][cursor : cursor + Session_path_size]
        )
    ...
    def AdjustTipToPiezoCenter(self, threshold=0.7):
        ...
        if Z_pos > Z_lim["high"] * threshold or Z_pos < Z_lim["low"] * threshold:
            ...
            self.Withdraw()
            if (
                Z_pos > Z_lim["high"] * threshold
            ):  # the tip is too close to the surface, so move the tip Z-
                Tip_move_direction = "Z-"
                gorl_Z_limit = [Z_lim["low"], Z_lim["low"] * 0.2]
            elif (
                Z_pos < Z_lim["low"] * threshold
            ):  # the tip is too far from the surface, so move the tip Z+
                Tip_move_direction = "Z+"
    ...
    def BiasSpectrStart(self, get_data=1, save_base_name=""):
        """
        Starts a bias spectroscopy measurement.
        
        Arguments:
        - get_data (bool): defines if the function returns the spectroscopy data (True) or not (False)
        - save_base_name (str): the basename used by the saved files. If empty string, there is no change
        ...
        - Parameters (1D array float32) returns the list of fixed parameters and parameters (in that order). To see
        the names of the returned parameters, use the BiasSpectr.PropsGet function.
        - Error described in the Response message>Body section
        """
        try:
            # Prepare arguments for send method
            # Arguments: get_data (uint32), save_base_name_size (int), save_base_name (string)
            args = [
                "BiasSpectr.Start",
                "uint32", int(get_data),
                "int", len(save_base_name),
            ]
            ...
            if not get_data:
                # If not getting data, just return success status
                return {"status": "started", "get_data": False}
            
            # Parse response if getting data
            cursor = 0
            
            # Validate response has minimum size
            if len(response["body"]) < 8:
                raise nanonisException(f"BiasSpectrStart failed: insufficient response data (got {len(response['body'])} bytes, need at least 8)")
            
            # Parse channels information
            channels_names_size = from_binary("int", response["body"][cursor:cursor + 4])
            cursor += 4
            
            if cursor + 4 > len(response["body"]):
                raise nanonisException(f"BiasSpectrStart failed: buffer too small for number_of_channels (cursor={cursor}, buffer_size={len(response['body'])})")
            ...
            for i in range(number_of_channels):
                if cursor + 4 > len(response["body"]):
                    raise nanonisException(f"BiasSpectrStart failed: buffer too small for channel {i} name size (cursor={cursor}, buffer_size={len(response['body'])})")
                    
                channel_name_len = from_binary("int", response["body"][cursor:cursor + 4])
                cursor += 4
                
                if cursor + channel_name_len > len(response["body"]):
                    raise nanonisException(f"BiasSpectrStart failed: buffer too small for channel {i} name (cursor={cursor}, name_len={channel_name_len}, buffer_size={len(response['body'])})")
                    
                channel_name = from_binary("string", response["body"][cursor:cursor + channel_name_len])
                channels_names.append(channel_name)
                cursor += channel_name_len
            
            # Parse data dimensions
            if cursor + 8 > len(response["body"]):
                raise nanonisException(f"BiasSpectrStart failed: buffer too small for data dimensions (cursor={cursor}, buffer_size={len(response['body'])})")
                
            data_rows = from_binary("int", response["body"][cursor:cursor + 4])
            cursor += 4
            data_columns = from_binary("int", response["body"][cursor:cursor + 4])
            cursor += 4
            
            # Parse data array
            expected_data_size = data_rows * data_columns * 4  # 4 bytes per float32
            if cursor + expected_data_size > len(response["body"]):
                raise nanonisException(f"BiasSpectrStart failed: buffer too small for data array (cursor={cursor}, expected_data_size={expected_data_size}, buffer_size={len(response['body'])})")
            
            data = np.zeros((data_rows, data_columns), dtype=np.float32)
            for i in range(data_rows):
                for j in range(data_columns):
                    data[i, j] = from_binary("float32", response["body"][cursor:cursor + 4])
                    cursor += 4
            
            # Parse parameters
            if cursor + 4 > len(response["body"]):
                raise nanonisException(f"BiasSpectrStart failed: buffer too small for number_of_parameters (cursor={cursor}, buffer_size={len(response['body'])})")
                
            number_of_parameters = from_binary("int", response["body"][cursor:cursor + 4])
            cursor += 4
            
            expected_params_size = number_of_parameters * 4  # 4 bytes per float32
            if cursor + expected_params_size > len(response["body"]):
                raise nanonisException(f"BiasSpectrStart failed: buffer too small for parameters array (cursor={cursor}, expected_params_size={expected_params_size}, buffer_size={len(response['body'])})")
            
            parameters = []
            for i in range(number_of_parameters):
                param_value = from_binary("float32", response["body"][cursor:cursor + 4])
                parameters.append(param_value)
                cursor += 4
            
            return {
                "channels_names_size": channels_names_size,
            ...
            error_msg = f"BiasSpectrStart failed: {str(e)}"
            logging.error(error_msg)
            raise nanonisException(error_msg)

        

    def BiasSpectrStop(self):
        self.send("BiasSpectr.Stop")

    def BiasSpectrChsSet(self, channel_indexes):
        """
        Sets the list of recorded channels in Bias Spectroscopy.

        Arguments:
        - channel_indexes (list of int): List of channel indexes to record.
          The indexes are comprised between 0 and 23 for the 24 signals assigned in the Signals Manager.
          To get the signal name and its corresponding index in the list of the 128 available signals
          in the Nanonis Controller, use the Signals.InSlotsGet function.

        Return arguments (if Send response back flag is set to True when sending request message):
        - Error described in the Response message>Body section
        """
        if not isinstance(channel_indexes, list):
            raise nanonisException("channel_indexes must be a list of integers.")

        # Validate channel indexes range
        for idx in channel_indexes:
            if not isinstance(idx, int) or idx < 0 or idx > 23:
                raise nanonisException(
                    "Channel indexes must be integers between 0 and 23."
                )

        # Prepare the command arguments
        args = ["BiasSpectr.ChsSet", "int", len(channel_indexes)]

        # Add each channel index to the arguments
        for idx in channel_indexes:
            args.extend(["int", idx])
    ...
    def BiasSpectrChsGet(self):
        """
        Gets the list of recorded channels in Bias Spectroscopy.
        ...
        - Channel indexes (1D array int) are the indexes of recorded channels.
          The indexes are comprised between 0 and 23 for the 24 signals assigned in the Signals Manager.
        """
        response = self.send("BiasSpectr.ChsGet")
        cursor = 0
        ...
        channel_indexes = []
        for i in range(number_of_channels):
            channel_index = from_binary("int", response["body"][cursor : cursor + 4])
            channel_indexes.append(channel_index)
            cursor += 4

        return {
            "Number of channels": number_of_channels,
            "Channel indexes": channel_indexes,
        }

    def BiasSpectrLimitsSet(self, start_value: float, end_value: float):
        """
        Arguments:
        Start value (V) (float32) is the starting value of the sweep
        End value (V) (float32) is the ending value of the sweep
        """
        self.send("BiasSpectr.LimitsSet", "float32", start_value, "float32", end_value)

    def BiasSpectrLimitsGet(self):
        """
        Gets the bias spectroscopy sweep limits.
        ...
        """
        parsedResponse = self.parse_response(
            self.send("BiasSpectr.LimitsGet"), "float32", "float32"
        )
        return {"Start value": parsedResponse["0"], "End value": parsedResponse["1"]}
    ...
    def BiasSpectrStatusGet(self):
        ...
        """
        parsedResponse = self.parse_response(
            self.send("BiasSpectr.StatusGet"), "uint32"
        )
        return {"Status": parsedResponse["0"]}
    
    def BiasSpectrPropsSet(self, save_all=0, number_of_sweeps=0, backward_sweep=1, 
                           number_of_points=301, z_offset=0.0, autosave=0, show_save_dialog=1):
        """
        Configures the Bias Spectroscopy parameters.
        """
        # Convert z_offset if it's a string
        if type(z_offset) is str:
            z_offset_val = self.convert(z_offset)
        else:
            z_offset_val = float(z_offset)
            
        self.send("BiasSpectr.PropsSet", 
                  "uint16", int(save_all),
                  "int", int(number_of_sweeps), 
                  "uint16", int(backward_sweep),
                  "int", int(number_of_points),
                  "float32", z_offset_val,
                  "uint16", int(autosave),
                  "uint16", int(show_save_dialog))

    def BiasSpectrPropsGet(self):
        """
        Gets the Bias Spectroscopy parameters and configuration.

        Arguments: None
        Return arguments (if Send response back flag is set to True when sending request message):
        - Save all (unsigned int16) where 1 means that the data from the individual sweeps is saved along with the
        average data of all of them, and 0 means that the individual sweeps are not saved in the file. This parameter
        only makes sense when multiple sweeps are configured
        - Number of sweeps (int) is the number of sweeps to measure and average
        - Backward sweep (unsigned int16), where 1 means that the backward sweep is performed (forward is
        always measured) and 0 means that there is no backward sweep
        ...
        - Channels (1D array string) returns the names of the acquired channels in the sweep. The size of each string
        item comes right before it as integer 32
        ...
        - Parameters (1D array string) returns the parameters of the sweep. The size of each string item comes right
        before it as integer 32
        ...
        - Error described in the Response message>Body section
        """
        response = self.send("BiasSpectr.PropsGet")
        cursor = 0
        
        # Parse basic parameters
        save_all = from_binary("uint16", response["body"][cursor:cursor + 2])
        cursor += 2
        number_of_sweeps = from_binary("int", response["body"][cursor:cursor + 4])
        cursor += 4
        backward_sweep = from_binary("uint16", response["body"][cursor:cursor + 2])
        cursor += 2
        number_of_points = from_binary("int", response["body"][cursor:cursor + 4])
        cursor += 4
        
        # Parse channels
        channels_size = from_binary("int", response["body"][cursor:cursor + 4])
        cursor += 4
        number_of_channels = from_binary("int", response["body"][cursor:cursor + 4])
        cursor += 4
        ...
        # Parse parameters
        parameters_size = from_binary("int", response["body"][cursor:cursor + 4])
        cursor += 4
        number_of_parameters = from_binary("int", response["body"][cursor:cursor + 4])
        cursor += 4
        ...
        for _ in range(number_of_parameters):
            param_name_len = from_binary("int", response["body"][cursor:cursor + 4])
            cursor += 4
            param_name = from_binary("string", response["body"][cursor:cursor + param_name_len])
            parameters.append(param_name)
            cursor += param_name_len
            
        # Parse fixed parameters
        fixed_parameters_size = from_binary("int", response["body"][cursor:cursor + 4])
        cursor += 4
        number_of_fixed_parameters = from_binary("int", response["body"][cursor:cursor + 4])
        cursor += 4
        
        fixed_parameters = []
        for _ in range(number_of_fixed_parameters):
            fixed_param_name_len = from_binary("int", response["body"][cursor:cursor + 4])
            cursor += 4
            fixed_param_name = from_binary("string", response["body"][cursor:cursor + fixed_param_name_len])
            fixed_parameters.append(fixed_param_name)
            cursor += fixed_param_name_len
        
        return {
            "Save all": save_all,
            "Number of sweeps": number_of_sweeps,
            "Backward sweep": backward_sweep,
            ...
            "Parameters": parameters,
            "Fixed parameters size": fixed_parameters_size,
            "Number of fixed parameters": number_of_fixed_parameters,
