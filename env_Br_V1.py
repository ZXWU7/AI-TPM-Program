...
class Molecule:
    def __init__(self, position=(0, 0), shape='square', size = 160, state=0, angle=0):
        ...
        if shape == 'circle':
            ...
            # interaction parameters
            self.key_points_size_factor = 0.6
            self.center_std_devs = 50
            self.center_weights = 1
            self.edge_std_devs = 45
            self.edge_weights = 0.6 
            ...
            self.reaction_path_num = 0 # 0 means 'shoulder' path, 1 means 'para' path
            ...
            # Chemical Synthesis path ：1. 0→1→2→4→5 (shoulder)   2. 0→1→3→4→5 (para)  
            self.optimal_parameters = {
                'full':     {'bias': 2.4,   'current': 0.15 },
                'one':      {'bias': 2.8,   'current': 0.15 },
                'shoulder': {'bias': 3.2,   'current': 0.2  },
                'para':     {'bias': 3.2,   'current': 0.25 },
                'single':   {'bias': 3.2,   'current': 0.3  },
                'empty' :   {'bias': 3.45,  'current': 1.0  },
...
class Env:
    def __init__(self,polar_space = False):
        ...
        if self.polar_space:
            ...
            action_low_x, action_high_x = -150, 150
            action_low_y, action_high_y = -150, 150


        self.action_space = spaces.Box(
            low=np.array([action_low_x, action_low_y, 2.3, 0.1]),       #low bias, low current
            high=np.array([action_high_x, action_high_y, 3.7, 1.1]),    #high bias, high current
            dtype=np.float32,
        )
    ...
    def step(self, action):
        ...
        for molecule in self.surface.molecules[0:]:
            state_init = molecule.molecular_state
