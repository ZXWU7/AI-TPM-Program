...
if __name__ == "__main__":
    ...
    tip_induce_mode = "CC"  # 'CC' or 'CH'  CC means Constant I   'CH' means Constant height  'pulse' means pulse mode

    SAC_init_mode = "new"  # 'new' or 'latest' new: create a new model, latest: load the latest checkpoint model
    ...
    while tip_in_boundary(
        ...
        if scan_qulity == 0:
            ...
            empty_count = 0
            tri_key_points_result, angle = nanonis.molecular_tri_seeker(
                nanonis.image_for,
                n=tri_size,
            ...
            while True:
                ...
                if molecule:  # molecule is not None
                    ...
                    init_scan_flag = 0
                    while (
                        not np.array_equal(molecule.site_states, np.array([2, 2, 2, 2]))
                        and 3 not in molecule.site_states
                    ):  # 分子没反应完并且没坏掉
                        ...
                        if init_scan_flag != 0:
                            ...
                            tracker_scale = nanonis.convert(
                                nanonis.scan_zoom_in_list[-1]
                            )
                            nanonis.TipXYSet(
                                nanonis.nanocoodinate[0],
                                nanonis.nanocoodinate[1] + 0.5 * tracker_scale,
                            )
                        ...
                        key_points_result = nanonis.single_molecular_seeker(
                            nanonis.image_for,
                            scan_posion=nanonis.nanocoodinate,
                        ...
                        if key_points_result == None:  #  No molecule
                            ...
                            no_mol_binary_arr = np.array([2])  # 2 means suc molecule
                        ...
                        H_site_state_list = np.array([int(key_points_result[0][0] + 1)])

                        # mols_center

                        binary_arr = H_site_state_list  # 1 is H, 2 no H, 3 is bad
                        # binary_arr = np.array(binary_arr[0]) # 0000 1111 2222
                        ...
                        Br_pos_nano = cal_Br_pos(
                            molecule.position, nanonis.mol_scale, angle
                        )
                        nanonis.molecule_registry.update_molecule(
                            molecular_index, Br_postion=Br_pos_nano
                        )
                        molecule = nanonis.molecule_registry.molecules[molecular_index]

                        # draw the XYedge frame in scan_for
                        image_for_rgb = cv2.cvtColor(
                            nanonis.image_for, cv2.COLOR_GRAY2BGR
                        )
                        square_side = (
                        ...
                        center = (
                            int(
                                key_points_result[0][5] * nanonis.scan_square_Buffer_pix
                            ),
                            int(
                                key_points_result[0][6] * nanonis.scan_square_Buffer_pix
                            ),
                        )
                        ...
                        #########################################################################
                            ...
                            (
                                ...
                                (voltage_click, current_click),
                                br_sites_array,
                            ) = mouse_click_tip(image_for_rgb, sigle_state=True)
                            ...
                            # state4SAC = agent.one_hot_encode(state4SAC)
                            ...
                            save_action_to_npy(
                                action_from_SAC,
                                file_path=nanonis.SAC_origin_action_path
                                + "/actions.npy",
                            )
                        ...
                        if result:  # molcule do not have to tip manipulation anymore
                            r_frac, theta_deg, dx_rot, dy_rot = (
                                point_in_rotated_rect_polar(
                                    result,
                                    (
                                        nanonis.scan_square_Buffer_pix,
                                        nanonis.scan_square_Buffer_pix,
                            ...
                            if tip_induce_mode == "CC":
                                ...
                                current_step = (
                                    tip_current - abs_tip_current_init
                                ) / steps
                                ...
                                for i in range(steps):
                                    # 逐步设置Bias和Setpoint
                                    nanonis.BiasSet(
                                        abs_tip_bias_init + bias_step * (i + 1)
                                    )
                                    nanonis.SetpointSet(
                                ...
                                # time.sleep(12)   # wait for the tip induce
                                nanonis.BiasPulse(tip_bias, 0.1)
                                time.sleep(1)  # wait for the tip induce
                                nanonis.BiasSet(tip_bias_init)
                                nanonis.SetpointSet(tip_current_init)
                                nanonis.ZCtrlOnSet()
                            ...
                            if len(temp_buffer) == 0:  # temp_buffer is empty
                                ...
                                action = np.array(
                                    [dx_rot, dy_rot, tip_bias, tip_current * 10**9]
                                )
                                ...
                                reward, info = nanonis.env.H_reward_culculate(
                                    state_legal, next_state_transform, 0
                                )
                                done = 0
                                if (
                                    2 in binary_arr
                                    or 3 in binary_arr
                                    or info["reaction"] in ["bad", "wrong", "skip"]
                                ):
                                    done = 1
                                ...
                                if tip_agent_mode == "SAC":
                                    ...
                                    if action_visualize:
                                        ...
                                        if (
                                            action_visualize_Queue.full()
                                        ):  # 如果队列满了，则等待一段时间
                                            action_visualize_Queue.get()

                                        action_visualize_Queue.put(
                                            (
                                                temp_buffer[0],
                                                temp_buffer[1],
                                                temp_buffer[2],
                        ...
                        # click the right mouse button
                            ...
                            if (
                                len(temp_buffer) == 0
                            ):  # the mol is no need to be manipulated in the first step
                                binary_arr = np.array([3])
                                nanonis.molecule_registry.update_molecule(
                                    molecular_index, site_states=binary_arr, status=2
                                )
                                molecule = nanonis.molecule_registry.molecules[
                                    molecular_index
                                ]
                            ...
                            if (
                                br_sites_array is not None
                            ):  # force the Br sites to be the new state that the user input
