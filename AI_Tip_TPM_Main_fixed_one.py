...
if __name__ == "__main__":
    ...
    lattice_shape = "triangle"  # 'triangle' or 'all'
    ...
    nanonis.nanocoodinate = (center_x, center_y)  # nanonis coordinate
                        ...
                        if tip_induce_mode == "pulse":
                            ...
                            for iteration in range(max_iterations):
                                ...
                                while time.time() - start_time < pre_pulse_duration:
                                    signal_Z = (
                                        nanonis.SignalValsGet(30)["0"] * 1e9
                                    )  # 获取信号值
                                    pre_pulse_signals.append(signal_Z)
                                    time.sleep(0.01)  # 防止过于频繁的采样

                                # 计算前0.5秒信号的平均值
                                pre_pulse_avg = sum(pre_pulse_signals) / len(
                                    pre_pulse_signals
                                )
                                ...
                                # 计算后0.5秒信号的平均值
                                post_pulse_avg = sum(post_pulse_signals) / len(
                                    post_pulse_signals
                                )
                                ...
                                print(
                                    f"Iteration {iteration + 1}: Pre-pulse Avg = {pre_pulse_avg}, Post-pulse Avg = {post_pulse_avg}, Difference = {signal_difference}"
                                )

                                # 判断是否满足中止条件
                                if signal_difference > threshold:
                                    print(
                                        "Signal difference exceeds threshold. Exiting loop."
                                    )
                                    break
