...
from env_TPM import Env  # 导入自定义环境
...
class SACAgent:
    def __init__(self, env: Env,
                 ...
                 gamma=0.99, 
                 tau=0.005, 
                 alpha=0.3,  # entropy temperature, dont be too large
                 lr=0.0003,
                 ):
    ...
    def train(self, num_steps, render=False, visualize=False, load_buffer=True, save_path = None, q_in: Queue = None,q_out: Queue = None, q_save_exp: Queue = None):
        ...
        if load_buffer:
            data_file = './train_result_plot/pre_buffer/Ni_buffer_data_V2.txt'
        ...
        for step in range(num_steps):
            ...
            ################# get from auto_scan ######################################
                ...
                temp_buffer_list = q_save_exp.get()

                
                if temp_buffer_list == 'wait': # the exp is not ready,dot block the process
                    continue
                
                else:           
                #  state, action, reward, next_state, done, info
                    for temp_buffer in temp_buffer_list:
                        state = temp_buffer[0]
                        ...
                        if int(temp_buffer[0][0]) == 1:
                            self.replay_buffer.add(temp_buffer[0],temp_buffer[1], temp_buffer[2], temp_buffer[3], temp_buffer[4], temp_buffer[5])
                        
                        if self.replay_buffer.size() > self.batch_size: # update the model every exp has been added
                            self.update() 
            else:
...
if __name__ == "__main__":
    ...
    # txt_path = 'D:/my_python_data/data_sum/0424/2025-04-24 11-15-44/SAC/SAC_buffer'
    ...
    # # load a npy file
    # action_data = np.load('C:/Users/73594/Desktop/my_python/Ni-TPM_for_AI/log/2025-04-24 11-15-44/SAC/SAC_origin_action/actions.npy', allow_pickle=True)
