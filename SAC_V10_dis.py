...
class DiscreteCriticNet(nn.Module):
    ...
    def forward(self, state, actions_index):
        ...
        one_hot_actions = torch.cat(one_hot_actions, dim=-1)  # [batch_size, sum(action_dims)]
...
class SACAgent:
    def __init__(self, env: Env, buffer_size=10000, 
                 batch_size=64, gamma=0.01, tau=0.005, alpha=0.2, lr=0.001,
                 action_dims=[15, 15, 15, 15]):
        ...
        self.target_entropy = -sum([np.log(a) for a in self.action_dims])
        ...
        self.alpha_optim = optim.Adam([self.log_alpha], lr=lr, weight_decay=1e-4)
    ...
    def remap_action(self, actions_index):
        """将离散动作索引映射到实际动作值"""
        if actions_index.ndim == 1:
            # shape [num_action_dims] => [1, num_action_dims]
            actions_index = actions_index.unsqueeze(0)

        mapped_actions = []
        for i in range(self.num_action_dims):
            # 假设每个动作维度的值在 [-1, 1] 之间均匀分布
            # discrete_levels = np.linspace(-1, 1, self.action_dims[i])
            discrete_levels = torch.linspace(-1, 1, steps=self.action_dims[i], device=self.device)
            
            # mapped = discrete_levels[actions_index[i].cpu().numpy()]
            mapped = discrete_levels[actions_index[:, i]]
            
            mapped_actions.append(mapped)

        # mapped_actions = torch.tensor(mapped_actions, device=self.device)
        mapped_actions = torch.stack(mapped_actions, dim=-1)

        mapped_actions = self.action_bias + self.action_scale * mapped_actions
        # mapped_actions = np.stack(mapped_actions, axis=-1)  # [batch_size, num_action_dims]

        # decrease the dimension of the mapped_actions
        
        mapped_actions = mapped_actions.squeeze(0)
        return mapped_actions
    ...
    def train(self, episode, render=False, visualize=False):
    ...
    def update(self):
        ...
        q_loss1 = F.smooth_l1_loss(current_q1, q_target)
        q_loss2 = F.smooth_l1_loss(current_q2, q_target)
