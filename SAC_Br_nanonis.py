...
class ReplayBuffer:
    ...
    def legalize_state(self, state):
        ...
        change_path_flag = 0
        # if have 2 in current ,return state ,"identity",change_path_flag
        if 2 in current:
            return state, "identity", change_path_flag
        # if current is (1,1,1,1) ,return state ,"identity",change_path_flag
        if current == (1, 1, 1, 1):
            return state, "identity", change_path_flag
                ...
                if current in self.legal_patterns:
                    ...
                    for t in self.transform_types:
                        ...
                        if tuple(transformed[:4]) in self.legal_patterns:
                            return transformed, t, change_path_flag
            else:
        ...
        # 如果没有匹配则更改最后一位数字，如果是0就变成1，如果是1就变成0，并且重新匹配
            state[-1] = 1 - state[-1]
            change_path_flag = 1
    ...
    def aug_exp(self, state, action, reward, next_state, done, info):
        ...
        """
        if tuple(state[:4]) == (1, 1, 1, 1) or 2 in state[:4]:
            return
        # 添加原始经验
        ...
        for t in aug_types:
            ...
            if info_aug["reaction"] not in ["bad", "wrong", "skip"]: 
                done = 0
            else:
                done = 1
    ...
    def plot_train(self, hyperparameters, save_path = None):
        ...
        if save_path is None:
            ...
            # add the hyperparameters to the figure name
            str_hyperparameters = '_'.join([f"{key}={value}" for key, value in hyperparameters.items()])
            plt.savefig(f"{save_path}/action_{self.now}_{str_hyperparameters}.png")
        # clear the figure
        plt.clf()
    # save the buffer data as a txt file
    def save(self, save_path = 'train_result_plot'):
        # now = time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time()))
        with open(save_path + f"/buffer_data_{self.now}.txt", "w") as f:
            for state, action, reward, next_state, done, info in self.buffer:
...
class SACAgent:
    def __init__(self, env: Env,
                 ...
                 gamma=0.5, 
                 tau=0.005, 
                 alpha=0.09,  # entropy temperature, dont be too large
                 lr=0.0002,
                 ):
        ...
        self.state_dim = 1 #env.observation_space.shape[0]                         # input dimension  Br → 5    H → 1 
    ...
    def train(self, num_steps, render=False, visualize=False, load_buffer=True, save_path = None, q_in: Queue = None,q_out: Queue = None, q_save_exp: Queue = None):
        ...
        if q_in is None:
            ...
            # state = self.one_hot_encode(state)  # 转换为独热编码并添加反应时间


        if load_buffer:
            data_file = './train_result_plot/pre_buffer/11.txt'
        ...
        for step in range(num_steps):
            ...
            state_legal, trans_type , change_path_flag = self.replay_buffer.legalize_state(state)

            # action,_ = self.select_action(state, detach_action=True)
            action, _ = self.select_action(state_legal, detach_action=True)
            # action = np.array([-180,-180,2.5,0.15])
            action_corrected = self.replay_buffer.inverse_transform_action(action, trans_type)

            # if the state is in the para path, then flip the y axis
            # if state_legal[-1] == 1:
            #     action_corrected[1] = -action_corrected[1]
            # connect to the env
            # next_state, reward, done, info = self.env.step(action)

            # transfer the action to the auto_scan



            ################# send to auto_scan ######################################
            if q_in is not None:
                # action_corrected = np.array([-180,-180,2.5,0.15])
                q_out.put(action_corrected)
              
            
            ################# get from auto_scan ######################################
                # if q_save_exp is not empty, then get the data from the queue
                ...
                temp_buffer = q_save_exp.get()

                
                if temp_buffer == 'wait': # the exp is not ready,dot block the process
                    continue
                
                else:           
                #  state, action, reward, next_state, done, info
                    state = temp_buffer[0]
                    action = temp_buffer[1]
                    reward = temp_buffer[2]
                    next_state = temp_buffer[3]
                    done = temp_buffer[4]
                    info = temp_buffer[5]
                    

                    # state = self.replay_buffer.legalize_state(state)
                    _, _ , change_path_flag = self.replay_buffer.legalize_state(next_state)
                    if change_path_flag == 1:
                        # change the last element of the state and next_state
                        # state[-1] = 1 - state[-1]

                        # next_state[-1] = 1 -next_state[-1]
                        # state[-1] = next_state[-1]

                        # reculculate the reward and info
                        reward, info = self.env.reward_culculate(state, next_state, 0)
                        if info["reaction"] not in ["bad", "wrong","skip"]:
                            done = 0
                        else:
                            done = 1
            ...
            self.replay_buffer.aug_exp(temp_buffer[0],temp_buffer[1], temp_buffer[2], temp_buffer[3], temp_buffer[4], temp_buffer[5])
            ...
            if step % 2 == 0:
                self.save_model(step, save_path = save_path)    
    ...
    def save_model(self, step, save_path = None):
        ...
        if save_path:
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            torch.save(self.policy_net.state_dict(), f"{save_path}/policy_net_{step}.pth")
            torch.save(self.q_net1.state_dict(), f"{save_path}/q_net1_{step}.pth")
            torch.save(self.q_net2.state_dict(), f"{save_path}/q_net2_{step}.pth")
        else:
