...
if __name__ == '__main__':
    ...
    tip_agent_mode = 'human'    # 'SAC' or 'human'
    ...
    nanonis.segmented_image_path = nanonis.image_data_save_path + '/segmented_image/'
    ...
    molecule = Molecule((Frame['center_x'], Frame['center_y']),None)
    ...
    while not np.array_equal(molecule.site_states, np.array([1, 1, 1, 1])) and 2 not in molecule.site_states: # 分子没反应完并且没坏掉
        ...
        binary_arr = [1 if state > nanonis.Br_site_threshold else 0 for state in Br_site_state_list]        # 1 is not Br, 0 is Br
        ...
        if result: # molcule do not have to tip manipulation anymore
            ...
            if len(temp_buffer)==0:    # temp_buffer is empty
                ...
                nanonis.buffer4aug.aug_exp(temp_buffer[0],temp_buffer[1],temp_buffer[2],temp_buffer[3],temp_buffer[4],temp_buffer[5])
