...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        success_ratio = 0.4
        # 剩下 latest_ratio、random_ratio 可以保持原逻辑
        latest_ratio = 0.5
        random_ratio = 0.1
        ...
        # 1) 找出所有“成功”样本 (不在 ["bad","wrong","non","skip"])
        success_all_idx = [
            idx for idx, exp in enumerate(self.buffer)
            if exp[-1].get("reaction") not in ["bad", "wrong", "non", "skip"]
        ]
        # 实际可用的成功样本数
        actual_success = min(num_success, len(success_all_idx))

        # 2) 根据反应类型分优先级（从高到低：4>3>2>1）
        cat4_idx = [i for i in success_all_idx if self.buffer[i][-1].get("reaction") == "mol 0→1"]
        cat3_idx = [i for i in success_all_idx if self.buffer[i][-1].get("reaction") in ["mol 1→2","mol 1→3"]]
        cat2_idx = [i for i in success_all_idx if self.buffer[i][-1].get("reaction") in ["mol 2→4","mol 3→4"]]
        cat1_idx = [i for i in success_all_idx if self.buffer[i][-1].get("reaction") == "mol 4→5"]

        # 3) 为四个优先级平分要抽取的数量
        #    若有余数，则从优先级最高的 cat4 开始依次 +1
        base_count = actual_success // 4
        remainder = actual_success % 4
        need4 = base_count + (1 if remainder > 0 else 0)
        need3 = base_count + (1 if remainder > 1 else 0)
        need2 = base_count + (1 if remainder > 2 else 0)
        need1 = base_count

        # 编写一个辅助函数，用来按顺序从若干列表中抽取所需数量
        def sample_with_priority(needed, priority_lists):
            sampled = []
            for lst in priority_lists:
                if needed <= 0:
                    break
                if len(lst) >= needed:
                    sampled += random.sample(lst, needed)
                    # 从原列表中移除已抽到的下标，避免重复
                    for s in sampled[-needed:]:
                        lst.remove(s)
                    needed = 0
                else:
                    # 若数量不够，全拿走
        ...
        # 先抽 cat4
        cat4_samples = sample_with_priority(need4, [cat4_idx, cat3_idx, cat2_idx, cat1_idx])
        # 再抽 cat3
        cat3_samples = sample_with_priority(need3, [cat3_idx, cat2_idx, cat1_idx])
        # 再抽 cat2
        cat2_samples = sample_with_priority(need2, [cat2_idx, cat1_idx])
        # 最后抽 cat1
        cat1_samples = sample_with_priority(need1, [cat1_idx])

        sampled_success_experiences = [self.buffer[i] for i in (cat4_samples + cat3_samples + cat2_samples + cat1_samples)]

        # 4) 接下来，可以像原先一样处理 latest / random 的抽取
        #    例如：
        sampled_latest_experiences = list(self.buffer)[-num_latest:] if num_latest > 0 else []
        remaining = batch_size - (len(sampled_success_experiences) + len(sampled_latest_experiences))
        if remaining > 0:
            # 这里保持原逻辑：排除已经抽出的下标，再抽 random
            exclude_indices = set(cat4_samples + cat3_samples + cat2_samples + cat1_samples)
            ...
            if len(available_indices) < remaining:
                ...
                chosen = random.sample(available_indices, remaining)
                sampled_random_experiences = [self.buffer[idx] for idx in chosen]
        else:
    ...
    def save(self):
        ...
        with open(f"train_result_plot/buffer_data_{self.now}.txt", "w") as f:
            for state, action, reward, next_state, done, info in self.buffer:
                f.write(f"state: {state}, action: {action}, reward: {reward}, next_state: {next_state}, done: {done}, info: {info}/n")
...
class SACAgent:
    def __init__(self, env: Env
        ...
        self.viz_buffer = ReplayBuffer(buffer_size,env)
    ...
    def train(self, num_steps, render=False, visualize=False, load_buffer=True):
        ...
        if load_buffer:
            data_file = './SAC_fine-tune/buffer_V2/V11.txt'
            self.replay_buffer.read(data_file)

        toltal_state = [np.array([0,0,0,0,0]),np.array([1,0,0,0,0]),np.array([1,0,0,0,1]),np.array([1,1,0,0,0]),np.array([1,0,1,0,1]),np.array([1,1,1,0,0])]
        count = 0
        for step in range(num_steps):
            ...
            if np.array_equal(state_legal[:4],np.array([0,0,0,0])): # mol 0 -> 1
                if action[0] < -52.5 and action[1] < -52.5 and action[2] < 2.7 and action[2] > 2.5 and action[3] > 0.5 and action[3] < 1.2:
                    reward = 8.0
                elif action[2] < 2.4 or action[3] < 0.5:
                    reward = -3.0
                elif action[2] > 2.8 or action[3] > 1.6:
                    reward = -5.0
                else:
                    reward = -3
            
            if np.array_equal(state_legal,np.array([1,0,0,0,0])): # mol 1 -> 2
                if action[0] > 52.5 and action[1] < -52.5 and action[2] < 3.0 and action[2] > 2.8 and action[3] > 0.8 and action[3] < 1.5:
                    reward = 8.0
                elif action[2] < 2.8 or action[3] < 0.5:
                    reward = -3.0
                elif action[2] > 3.2 or action[3] > 1.6:
                    reward = -5.0
                else:
                    reward = -3
            
            if np.array_equal(state_legal,np.array([1,0,0,0,1])): # mol 1 -> 3
                if action[0] > 52.5 and action[1] > 52.5 and action[2] < 3.1 and action[2] > 2.8 and action[3] > 0.9 and action[3] < 1.5:
                    reward = 8.0
                elif action[2] < 2.8 or action[3] < 0.9:
                    reward = -3.0
                elif action[2] > 3.2 or action[3] > 1.5:
                    reward = -5.0
                else:
                    reward = -3

            
            if np.array_equal(state_legal[:4],np.array([1,1,0,0])): # mol 2 -> 4
                if action[0] > 52.5 and action[1] > 52.5 and action[2] < 3.1 and action[2] > 2.7 and action[3] > 0.9 and action[3] < 1.5:
                    reward = 8.0
                elif action[2] < 2.7 or action[3] < 0.9:
                    reward = -3.0
                elif action[2] > 3.3 or action[3] > 1.5:
                    reward = -5.0
                else:
                    reward = -3


            if np.array_equal(state_legal[:4],np.array([1,0,1,0])): # mol 3 -> 4
                if action[0] > 52.5 and action[1] < -52.5 and action[2] < 3.1 and action[2] > 2.8 and action[3] > 0.9 and action[3] < 1.5:
                    reward = 8.0
                elif action[2] < 2.8 or action[3] < 0.9:
                    reward = -3.0
                elif action[2] > 2.6 or action[3] > 1.5:
                    reward = -5.0
                else:
                    reward = -3

            if np.array_equal(state_legal[:4],np.array([1,1,1,0])): # mol 4 -> 5
                if action[0] < -52.5 and action[1] > 52.5 and action[2] < 3.3 and action[2] > 3.1 and action[3] > 0.8 and action[3] < 1.2:
                    reward = 8.0
                elif action[2] < 3.1 or action[3] < 0.8:
                    reward = -3.0
                elif action[2] > 3.5 or action[3] > 1.2:
                    reward = -5.0
                else:
                    reward = -3
            ...
            self.replay_buffer.add(state_legal, action, reward, next_state_SAC, done, info)
            # self.replay_buffer.aug_exp(state_legal, action, reward, next_state_SAC, done, info)
            self.viz_buffer.add(state_legal, action, 0, next_state_SAC, done, info)
            ...
            # change the state to the next state cycle
            count += 1
            if count == len(toltal_state):
                count = 0
            state = toltal_state[count]
            
        if visualize:
            queue.put(None)
            plot_process.join()
        
        self.viz_buffer.plot_train(self.hyperparameters)
        self.viz_buffer.save()
    ...
    def load_model(self, model_dir, step=None):
        """
        从指定目录加载某一步训练保存的网络权重，或自动加载最新step模型文件
        参数:
            model_dir: 保存模型文件的文件夹路径 (例如: "models/2025-03-22_22-00-00")
            step: 对应保存文件名中的 step；若不指定则加载最新的step
        """
        import re

        # 如果未指定 step，自动寻找最新 step
        if step is None:
            policy_files = []
            for fname in os.listdir(model_dir):
                # 匹配形如 "policy_net_数字.pth" 的文件名，以提取 step
                match = re.match(r"policy_net_(/d+)/.pth", fname)
                if match:
                    policy_files.append(int(match.group(1)))
            
            if not policy_files:
                print(f"未在 {model_dir} 找到有效的 policy_net_xxx.pth 文件，无法加载模型。")
                return
            
            step = max(policy_files)
            print(f"未指定 step，自动使用最新 step={step} 来加载模型。")

        policy_path = os.path.join(model_dir, f"policy_net_{step}.pth")
        q_net1_path = os.path.join(model_dir, f"q_net1_{step}.pth")
        q_net2_path = os.path.join(model_dir, f"q_net2_{step}.pth")

        self.policy_net.load_state_dict(torch.load(policy_path, map_location=self.device))
        self.q_net1.load_state_dict(torch.load(q_net1_path, map_location=self.device))
        self.q_net2.load_state_dict(torch.load(q_net2_path, map_location=self.device))

        self.target_q_net1.load_state_dict(self.q_net1.state_dict())
        self.target_q_net2.load_state_dict(self.q_net2.state_dict())

        print(f"Model loaded from {model_dir}, step={step}.")
    ...
    def plot_actions(self,queue,env):
        ...
        while True:
            ...
            # if state is np.array([0,0,0,0,0]) change the color to 半透明橙色
            if np.array_equal(state,np.array([0,0,0,0,0])):
                color = (1, 0.5, 0, 0.5) # 半透明橙色
            if np.array_equal(state,np.array([1,0,0,0,0])):
                color = (1, 1, 0, 0.5)  # 半透明黄色
            if np.array_equal(state,np.array([1,1,0,0,0])):
                color = (0, 1, 0, 0.5) # 半透明绿色
            if np.array_equal(state,np.array([1,1,1,0,0])):
                color = (0, 1, 1, 0.5) # 半透明青色
            if np.array_equal(state,np.array([1,0,1,0,1])):
                color = (0.5, 0, 1, 0.5) # 半透明紫色
            if np.array_equal(state,np.array([1,0,0,0,1])):
                color = (0, 0.5, 1, 0.5) # 半透明蓝色
...
if __name__ == "__main__":
    ...
    agent = SACAgent(env)
    # get all the txt path in the folder
    txt_path = 'D:/my_python_data/data_sum/0326/2025-03-26 00-47-44/SAC/SAC_aug_buffer'
    # read all the txt file in the folder
    for file in os.listdir(txt_path):
        if file.endswith(".txt"):
            file_path = os.path.join(txt_path, file)
            agent.replay_buffer.read(file_path)
    


    for i in agent.replay_buffer.buffer:
        # legalize the state
        # state_legal, trans_type, change_path_flag = agent.replay_buffer.legalize_state(i[0])
        # action_for_buffer = agent.replay_buffer.transform_action(i[1], trans_type)
        # if i[3][4] == 1:
        #     i[0][4] = 1
        if i[0][4] == 1:
            i[3][4] = 1
        ...
        R, info = env.reward_culculate(i[0], i[3], 0)

        # if np.array_equal(i[0][:4],np.array([1,1,0,0])) and np.array_equal(i[3][:4],np.array([1,1,1,0])): # mol 2 -> 4
        #     info["reaction"] = "mol 2→4"
        if info["reaction"] in ["bad", "wrong","skip"]:
            done = 1
        else:
            done = 0
        # if i[0][4] == 1:
        ...
        if info["reaction"] in ["non"]:
            ...
            # if i[1][0] < i[1][1] :
            # i[1][0] = -i[1][0]
            agent.viz_buffer.add(i[0],i[1],R,i[3],done,info)

    # agent.replay_buffer.read('D:/my_python_data/data_sum/0326/2025-03-26 00-47-44/SAC/SAC_aug_buffer/buffer_data_2025-03-26_00-47-44.txt')
    # agent.replay_buffer.read('D:/my_python_data/data_sum/0326/2025-03-26 00-47-44/SAC/SAC_aug_buffer/buffer_data_2025-03-26_01-13-38.txt')
    # agent.load_model("./SAC/train_results/2025-03-26 00-47-44", step = 2)
    # agent.train(5000,visualize=True)
    agent.viz_buffer.plot_train(agent.hyperparameters)
    
    
    
    # agent.replay_buffer.read('./SAC_fine-tune/buffer_V2/V10.txt')
    # agent.replay_buffer.plot_train(agent.hyperparameters)
    ...
    #         i[3][4] = 1
    #     agent.viz_buffer.add(i[0],i[1],R,i[3],done,info)
    # agent.viz_buffer.save()    
    # agent.viz_buffer.plot_train(agent.hyperparameters)
    



    #     if abs(i[1][0]) <= 80 and abs(i[1][1]) <= 80 and abs(i[1][2]) <= 4.1 and abs(i[1][3]) <= 2:
    #         # print(i[2])
    #         new_reward = i[2] * 10
    #         agent.viz_buffer.add(i[0],i[1],new_reward,i[3],i[4],i[5])
    # 
    #     # agent.viz_buffer.save()
   
   
    # agent.replay_buffer.read('./SAC_fine-tune/buffer_V2/Br_buffer_data_V7.txt')
    # agent.viz_buffer.plot_train(agent.hyperparameters)
