...
class Mustard_AI_Nanonis(NanonisController):
    def __init__(self, *args, **kwargs):
        ...
        self.scan_zoom_in_list = ["30n", "10n"]  # the list of the zoom in scan adge
        ...
        self.zoom_in_tip_speed = "5n"  # the speed of the zoom in tip

        self.scan_square_Buffer_pix = 400  # scan pix
        ...
        self.quality_model_path = (
            "./EvaluationCNN/CNN.pth"  # the path of the quality model weights
        )

        self.segment_model_path = "./mol_segment/unet_model-zzw-Ni_V2.pth"  # the path of the segment model weights

        self.keypoint_model_path = "./keypoint/best.pt"
    ...
    def key_points_convert(self, key_points_result):
        out_put_key_list = []
        for key_point in key_points_result:
            # key_points_result is [[class, x, y, w, h, key_x, key_y], ......]
            key_point[1] = self.inter_closest[0] + (key_point[1] - 0.5) * int(
                self.convert(self.scan_zoom_in_list[0]) * 10**9
            )
            key_point[2] = self.inter_closest[1] + (key_point[2] - 0.5) * int(
                self.convert(self.scan_zoom_in_list[0]) * 10**9
            )
            key_point[3] = key_point[3] * int(
                self.convert(self.scan_zoom_in_list[0]) * 10**9
            )
            key_point[4] = key_point[4] * int(
                self.convert(self.scan_zoom_in_list[0]) * 10**9
            )
            for i in range(5, len(key_point), 2):
                key_point[i] = self.inter_closest[0] + (key_point[i] - 0.5) * int(
                    self.convert(self.scan_zoom_in_list[0]) * 10**9
                )  # the key point x in pixel
                key_point[i + 1] = self.inter_closest[1] + (
                    key_point[i + 1] - 0.5
                ) * int(
                    self.convert(self.scan_zoom_in_list[0]) * 10**9
                )  # the key point y in pixel
                out_put_key_list.append(key_point)
        return out_put_key_list

    # def a function to predict the scan qulity
    def molecular_tracker(self, zoom_scale="30n"):
        ...
        key_points_result = key_detect(
            self.image_for, self.keypoint_model_path, key_point_save_dir
        )
        # regsitrated the all moleculars in a regsitration list
        if len(key_points_result) == 0:
            print("No molecular is detected")
            return 0
        self.molecular_registration_list = self.key_points_convert(key_points_result)
        self.molecular_registration_list = filter_close_bboxes(
            self.molecular_registration_list, self.molecular_filter_threshold
        )

        # move the tip to the first molecular
        # if zoom_scale == scan_zoom_in_listp[0], choose the first molecular as the center of the scan
        if zoom_scale == self.scan_zoom_in_list[0]:
            box_center_nano = pix_to_nanocoordinate(
                (
                    self.molecular_registration_list[0][1],
                    self.molecular_registration_list[0][2],
                ),
                plane_size=self.plane_size,
                int_prosses=False,
            )
        else:
            # if zoom_scale != scan_zoom_in_listp[0], choose the most center molecular as the center of the scan
            # sort the molecular_registration_list by the distance to the self.inter_closest
            self.molecular_registration_list = sorted(
                self.molecular_registration_list,
                key=lambda x: (x[1] - self.inter_closest[0]) ** 2
                + (x[2] - self.inter_closest[1]) ** 2,
            )
            box_center_nano = pix_to_nanocoordinate(
                (
                    self.molecular_registration_list[0][1],
        ...
        self.FolMeSpeedSet("2n", 1)
        # move the tip to the first row of the molecular
        self.TipXYSet(
            box_center_nano[0], box_center_nano[1] + self.convert(zoom_scale) / 2
        )
        # set back the tip speed to scan speed
        self.FolMeSpeedSet("2n", 0)

        return box_center_nano
...
if __name__ == "__main__":
    ...
    while tip_in_boundary(
        ...
        if scan_qulity == 0:
            ...
            for zoom_in in nanonis.scan_zoom_in_list:
                molecular_nano = nanonis.molecular_tracker(zoom_scale=zoom_in)
                if molecular_nano:
                    nanonis.batch_scan_producer(
                        molecular_nano, zoom_in, nanonis.scan_square_Buffer_pix, 0
                    )
                else:
                    # delete the molecular_nano from the molecular_registration_list
                    # nanonis.molecular_registration_list.pop(0)
                    break
